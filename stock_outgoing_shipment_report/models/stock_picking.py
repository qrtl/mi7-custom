...
class StockPicking(models.Model):
    ...
    def generate_stock_outgoing_shipment_report(self):
        if any(self.mapped('is_exported')):
            raise UserError(_("Following records have been exported before. Please uncheck 'Exported' if re-export is needed."))

        moves = self.mapped("move_lines")
        exist_data_lines = self.env["stock.outgoing.shipment.report"].search([('move_id', 'in', moves.ids)])
        for move in moves:
